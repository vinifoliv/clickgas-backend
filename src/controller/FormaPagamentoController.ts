    ...
    constructor(
        ...
        this.httpServer.post("/forma-pagamento", async (req, res) => {
            ...
            const formaPagamentoExiste =
                await this.formaPagamentoModel.buscarPorNome(
                    formaPagamento.nome
                );
            ...
            const formaPagamentoCriada = await this.formaPagamentoModel.criar(
                formaPagamento
            );
        ...
        this.httpServer.get("/forma-pagamento", async (_, res) => {
            const formasPagamento = await this.formaPagamentoModel.buscar();
        ...
        this.httpServer.put("/forma-pagamento/:id", async (req, res) => {
            ...
            const formaPagamento = this.montarFormaPagamento(req.body);
            const formaPagamentoExiste =
                await this.formaPagamentoModel.buscarPorId(id);
            if (!formaPagamentoExiste) {
                res.status(404).json({
                    message: "Forma de pagamento não encontrada.",
                });
            ...
            const formaPagamentoAlterada =
                await this.formaPagamentoModel.alterar(id, formaPagamento);
        ...
        this.httpServer.delete("/forma-pagamento/:id", async (req, res) => {
            ...
            const formaPagamentoExcluida =
                await this.formaPagamentoModel.excluir(id);
